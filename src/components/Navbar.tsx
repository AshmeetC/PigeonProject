    ...
    <>
      ...
      <nav className="md:hidden w-screen fixed top-0 right-0 p-4">
        ...
        >
          ...
          <button onClick={toggleDropdown} className="text-2xl focus:outline-none">
            &#9776;
          </button>
        </div>

        {isDropdownOpen && (
          <div className="absolute top-16 right-0 bg-[#625A4F] p-4 shadow-md mx-4">
            {['Home', 'Team', 'Courses'].map((item, index) => (
              <Link
                key={index}
                to={`/${item.toLowerCase()}`}
                className="block py-2 hover:font-extrabold"
